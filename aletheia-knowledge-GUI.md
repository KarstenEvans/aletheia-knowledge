...
Put books, gifts and affiliate-ready merchant links in resource HTML, not in factual card content. **Every public Aletheia Knowledge HTML page carries the Awin Publisher MasterTag for publisher `3182162` exactly once, immediately before `</body>`.** This site-wide rule keeps future monetisable links and Awin Convert-a-Link support consistent without having to remember which page may gain an advertiser link later. Do not put the MasterTag inside a visible footer fragment and do not add it to Markdown. The current project uses `data-awinignore` for factual/source links that must retain attribution or avoid conversion where supported; verify actual converter behaviour. Make page-level affiliate disclosures clear near commercial material and maintain an Affiliate Disclosure link.
